...
1.  회원가입 API
    ...
    -   메서드: POST
    -   설명: 회원가입 요청 시 내부적으로 이메일 형식 검증, 이메일 중복 확인, 비밀번호 강도 확인, 연락처 형식 검증, 연락처 중복 확인 등 모든 검증이 자동으로 이루어집니다. 별도의 중복 확인 API 호출이 필요하지 않습니다.
    -   요청 본문:
        ```json
        {
          "registerRequest": {
...
- [ ] 엔드포인트 개발 (회원가입 API)
- [ ] 회원 정보 등록 로직 구현
- [ ] 회원가입 요청 처리 시 내부적으로 수행되는 검증 로직 구현
    - [ ] 이메일 형식 검증 (RFC 5322)
    - [ ] 이메일 중복 확인
